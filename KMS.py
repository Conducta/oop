...
def init_db():
    ...
    if "active" not in cols:
        c.execute("ALTER TABLE borrow ADD COLUMN active INTEGER DEFAULT 1")  # 1 = active, 0 = deleted
    conn.commit()
    conn.close()
...
class KitchenInventoryApp(tk.Tk):
    ...
    def open_edit_item(self):
        ...
        def on_update():
            ...
            qty_diff = new_qty - cur_qty  # positive if increasing, negative if decreasing
            ...
            if new_item != cur_item:
                ...
                for inv in self.inventory_items:
                    if inv['name'] == cur_item:
                        ...
                        self.update_item_db(inv['id'], inv['name'], inv['quantity'])
                        break
                # subtract new qty from new item
                updated = False
                for inv in self.inventory_items:
                    if inv['name'] == new_item:
                        if inv['quantity'] >= new_qty:
                            ...
                            break
                if not updated:
                    messagebox.showerror("Error", "Not enough stock for new item.")
                    return
            else:
                # item didn't change, just adjust inventory by the difference
                for inv in self.inventory_items:
                    if inv['name'] == new_item:
                        if inv['quantity'] >= qty_diff:  # check if enough stock
                            inv['quantity'] -= qty_diff
                            self.update_item_db(inv['id'], inv['name'], inv['quantity'])
                            break
                        else:
                            messagebox.showerror("Error", "Not enough stock to increase quantity.")
                            return
        ...
        def on_delete():
            if messagebox.askyesno("Delete", "Delete this record? Stock will be returned."):
                # Only add back the outstanding quantity (borrowed - returned)
                outstanding_qty = cur_qty - (next((l['returned_qty'] for l in self.logs if l['db_id'] == db_id), 0))
                if outstanding_qty > 0:
                    for inv in self.inventory_items:
                        if inv['name'] == cur_item:
                            inv['quantity'] += outstanding_qty
                            self.update_item_db(inv['id'], inv['name'], inv['quantity'])
                            break

                conn = get_db_connection()
                c = conn.cursor()
                c.execute("UPDATE borrow SET active=0 WHERE id=?", (db_id,))
                conn.commit()
                conn.close()
