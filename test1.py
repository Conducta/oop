...
@ -65,29 +251,20 @@ class KitchenInventoryApp(tk.Tk):
            y_offset += 50
        ...
        self.logout_btn = tk.Button(
            ...
            width=15,
            height=2,
            self.sidebar, text="Log Out", bg="#8a543f", fg="white",
            font=("Arial", 12, "bold"), relief="flat", width=15, height=2,
            command=self.logout
        )  
        )
    ...
    def create_main(self):
        ...
        header.pack(pady=15)
@ -95,29 +272,57 @@ class KitchenInventoryApp(tk.Tk):
...
@ -130,7 +335,6 @@ class KitchenInventoryApp(tk.Tk):
...
@ -140,17 +344,16 @@ class KitchenInventoryApp(tk.Tk):
...
@ -196,6 +399,7 @@ class KitchenInventoryApp(tk.Tk):
                if inv['name'] == item_name:
                    if inv['quantity'] >= qty_int:
                        ...
                        log_entry = {
                            'borrower': name,
@ -208,6 +412,7 @@ class KitchenInventoryApp(tk.Tk):
                            'status': 'Borrowed'
                        }
                        ...
                        pop_up.destroy()
                        return
@ -220,8 +425,6 @@ class KitchenInventoryApp(tk.Tk):
        pop_up.grab_set()
        pop_up.wait_window()
...
@ -233,7 +436,7 @@ class KitchenInventoryApp(tk.Tk):
        pop_up.config(bg="#e5d6cc")
...
@ -258,7 +461,7 @@ class KitchenInventoryApp(tk.Tk):
                return
            sel_iid = sel[0]
            ...
            id_no = vals[1]
            ys = vals[2]
@ -269,17 +472,15 @@ class KitchenInventoryApp(tk.Tk):
            ...
            self.logs.append({
                'borrower': borrower,
                'id_no': id_no,
@ -288,10 +489,39 @@ class KitchenInventoryApp(tk.Tk):
                'qty': returned_qty,
            ...
            pop_up.destroy()
@ -301,21 +531,19 @@ class KitchenInventoryApp(tk.Tk):
        pop_up.grab_set()
        pop_up.wait_window()
...
@ -332,161 +560,155 @@ class KitchenInventoryApp(tk.Tk):
    ...
    def open_invtry_items(self):
        ...
        def refresh_inventory():
            ...
            for item in self.inventory_items:
                ...
                # Use string version of id as iid so we can retrieve it easily
        ...
        def edit_item():
            ...
            # Find the item in memory list using the iid (which is the id)
...
class KitchenInventoryApp(tk.Tk):
        ...
        logs_tree = ttk.Treeview(pop, columns=cols, show="headings", selectmode="extended")  # allow multiple selection
        for c in cols:
            logs_tree.heading(c, text=c)
            logs_tree.column(c, anchor="center", width=110)
@ -502,20 +724,40 @@ class KitchenInventoryApp(tk.Tk):
...
@ -523,37 +765,73 @@ class KitchenInventoryApp(tk.Tk):
                return
    ...
    def logout(self):
        # Ensure any open popups (logs/history, inventory, etc.) are closed before logging out.
    ...
    def create_popup(self, title):
        pop = tk.Toplevel(self)
        pop.title(title)
@ -564,7 +842,198 @@ class KitchenInventoryApp(tk.Tk):
        pop.grab_set()
        pop.wait_window()
    ...
    def open_generate_qr_window(self):
        ...
        def save_and_generate():
            ...
            messagebox.showinfo("Success", f"QR Code generated & saved to {file_path}")
            win.destroy()
