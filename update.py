...
class KitchenInventoryApp(tk.Tk):
    ...
    def open_borrowed_items(self):
        ...
        # Department (TOP)
        tk.Label(frame, text="Department:", bg="#e5d6cc",
                 font=("Arial", 12, "bold")).grid(row=0, column=0, sticky="w", pady=10)
        tk.Label(frame, text="BSHM", bg="#e5d6cc",
                 fg="#6b3f2c", font=("Arial", 12, "bold")).grid(row=0, column=1, sticky="w", pady=10)

        # Borrower Type (TOP)
        tk.Label(frame, text="Borrower Type:", bg="#e5d6cc",
        ...
        tk.Radiobutton(type_frame, text="Student", variable=borrower_type,
                       value="Student", bg="#e5d6cc").pack(side="left", padx=10)
        tk.Radiobutton(type_frame, text="Instructor", variable=borrower_type,
                       value="Instructor", bg="#e5d6cc").pack(side="left", padx=10)

        # Borrower Name
        tk.Label(frame, text="Borrower Name:", bg="#e5d6cc").grid(row=2, column=0, sticky="w", pady=10)
        name_entry = tk.Entry(frame, width=40)
        name_entry.grid(row=2, column=1, pady=10)

        # ID No
        tk.Label(frame, text="ID No.:", bg="#e5d6cc").grid(row=3, column=0, sticky="w", pady=10)
        id_entry = tk.Entry(frame, width=40)
        id_entry.grid(row=3, column=1, pady=10)

        # Year & Section
        tk.Label(frame, text="Year & Section:", bg="#e5d6cc").grid(row=4, column=0, sticky="w", pady=10)
        ys_entry = tk.Entry(frame, width=40)
        ys_entry.grid(row=4, column=1, pady=10)

        # Item
        tk.Label(frame, text="Item Borrowed:", bg="#e5d6cc").grid(row=5, column=0, sticky="w", pady=10)
        item_var = tk.StringVar()
        ttk.Combobox(frame, textvariable=item_var, state="readonly",
                     values=[i['name'] for i in self.inventory_items]).grid(row=5, column=1, pady=10)

        # Quantity
        tk.Label(frame, text="Quantity:", bg="#e5d6cc").grid(row=6, column=0, sticky="w", pady=10)
        qty_entry = tk.Entry(frame, width=40)
        qty_entry.grid(row=6, column=1, pady=10)

        # Date Borrowed
        tk.Label(frame, text="Date Borrowed:", bg="#e5d6cc").grid(row=7, column=0, sticky="w", pady=10)
        date_entry = tk.Entry(frame, width=40)
        date_entry.insert(0, datetime.date.today().isoformat())
        date_entry.grid(row=7, column=1, pady=10)

        tk.Button(pop_up, text="Submit", bg="brown", fg="white",
